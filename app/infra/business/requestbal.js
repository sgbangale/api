  ...
  createRequest: async req => {
    ...
    try {
      if (req.request_type === "request__view") {
        ...
        result.request_output_data = await require("../../businessLogic")[
          req.request_type
        ](req);
      }
      ...
      if (e.name === "MongoError" && e.code == 11000) {
        result.request_output_data =
          "Cannot create duplicate record in the system.";
      } else {
    ...
    return common.helper.respondMaker(
      result.request_status === "FAILED" ?
      "Error Ocurred while processing request." :
      "",
      result.request_output_data,
      result.request_status === "FINISHED"
    );
